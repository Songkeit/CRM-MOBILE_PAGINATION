package com.ezatpanah.hilt_retrofit_paging_youtube.Normal.NormalViewModel
...
class NormalViewModel @Inject constructor(
    ...
    //Api
    val detailsNormal = MutableLiveData<ApiInformationUser.Data?>()
    fun loadDetailsNormal(id: Int) = viewModelScope.launch {// ข้อมูลที่ถูกส่งไปที่ที่ detail
        loading.postValue(true)
        val response = repository.getUserDetails(id)
        if (response.isSuccessful) {
            detailsNormal.postValue(response.body()!!.data)
        }
